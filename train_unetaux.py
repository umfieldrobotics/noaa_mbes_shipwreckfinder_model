...
def train(train_path, val_path, model_name, model_arch, save_path, num_epochs, batch_size, lr, l1, using_hillshade, using_inpainted, scheduler_type='step'):
    model = UnetAux(2, 2)
    model.to(DEVICE)
    ...
    for epoch in tqdm(range(num_epochs), total=num_epochs, desc="Epochs"):
        ...
        for data in train_loader:
            ...
            seg_label = data['label'].to(DEVICE)
            
            optim.zero_grad()

            seg_pred = model(image)
            seg_loss = ce_loss(seg_pred, seg_label.long())
            seg_pred = seg_pred.argmax(dim=1)
            
            loss = seg_loss

            seg_label = seg_label.cpu().detach().numpy()
            seg_pred = seg_pred.cpu().detach().numpy()
            valid_data_mask = (seg_label.flatten() != -1)  # Ignore invalid pixels
            seg_label_flat, seg_pred_flat = seg_label.flatten()[valid_data_mask], seg_pred.flatten()[valid_data_mask]
            iou = jaccard_score(seg_label_flat, seg_pred_flat, zero_division=1)
            loss.backward()
            optim.step()
        ...
        if epoch % 25 == 0:
            with torch.no_grad():
                for idx, data in enumerate(val_loader):
                    ...
                    valid_data_mask = (seg_label.flatten() != -1)  # Ignore 255
                    seg_label_flat, seg_pred_flat = seg_label.flatten()[valid_data_mask], seg_pred.flatten()[valid_data_mask]
                    iou = jaccard_score(seg_label_flat, seg_pred_flat, zero_division=1)
                    ...
                    for j in range(len(seg_label)):
                        ...
                        wandb.log({"Val_Epoch": epoch, 'VAL__' + data['metadata']['image_name'][j].split('/')[-1]: wandb.Image(np.hstack([image[j].permute(1, 2, 0).cpu().numpy()[:,:,0], 
                                                                                                    np.ones((seg_label[j].shape[0], 5)), 
                                                                                                    seg_label[j], 
                                                                                                    np.ones((seg_label[j].shape[0], 5)), 
                                                                                                    image[j].permute(1, 2, 0).cpu().numpy()[:,:,1],
                                                                                                    np.ones((seg_label[j].shape[0], 5)), 
                                                                                                    seg_pred[j]]),
                                                                                                    caption="Input | Ground Truth | Hillshade | Prediction")})
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            if save_images:
                ...
                combined_img = save_combined_image(image[0,0,:,:], pred, label[0], data['metadata']['image_name'][0], pred_path, iou_ship)
...
if __name__ == "__main__":
    ...
    BATCH_SIZE = 64
    LEARNING_RATE = 5e-4
    L1 = 0.5 # weight for hillshade in loss
    USING_HILLSHADE = True
